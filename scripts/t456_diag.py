...
def task_t6():
    ...
    spans = info["holding_spans"]
    # 逐日股票持仓市值(近似): 入场权重×价格路径×初始资金, 按持仓期展开
    dates = panel.index[panel.index >= pd.Timestamp(START)]
    sh_val = pd.Series(0.0, index=dates)
    sz_val = pd.Series(0.0, index=dates)
    for code, ed, xd, w, ep in spans:
        if code not in panel.columns:
            continue
        seg = panel.loc[ed:xd, code].ffill().fillna(0.0)
        if ep and ep > 0:
            val = w * (seg / ep) * cfg.initial_capital
        else:
            val = w * cfg.initial_capital
        if str(code).startswith("6"):
            sh_val = sh_val.add(val, fill_value=0.0)
        else:
            sz_val = sz_val.add(val, fill_value=0.0)
    tot = sh_val + sz_val
    # bear清仓影响: 20交易日滚动日均(T-2~T-21口径用20日均近似)
    roll20 = tot.rolling(20).mean()
    print(f"A3 股票持仓市值(时间加权): 总体日均{tot.mean()/1e4:.0f}万, "
          f"沪{sh_val.mean()/1e4:.0f}万 深{sz_val.mean()/1e4:.0f}万")
    print(f"市值>0的交易日占比: {(tot > 0).mean()*100:.0f}% "
          f"(bear清仓段{(tot == 0).mean()*100:.0f}%)")
    print(f"20日滚动日均市值: 中位{roll20.median()/1e4:.0f}万, "
          f"低于1万(沪门槛)占比{(roll20 < 1e4).mean()*100:.0f}%")
    ...
    with open("logs/t6_daily_stock_mv_summary.log", "w", encoding="utf-8") as f:
        f.write(f"总日均{tot.mean()/1e4:.0f}万 沪日均{sh_val.mean()/1e4:.0f}万 "
                f"深日均{sz_val.mean()/1e4:.0f}万\n")
        f.write(f"零市值日占比{(tot == 0).mean()*100:.0f}% 20日滚动均市值中位"
                f"{roll20.median()/1e4:.0f}万\n")
