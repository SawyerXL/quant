...
NaN 段语义（2026-09-08 用户定案，显式定义，禁止 fillna/前值填充）：
  NaN 段内的股票: ①不可交易（apply_filters 排除、TP/止损/MA10 检查跳过）;
  ②不计入池（apply_filters 首个价格检查即剔除）; ③持仓日 MTM 零贡献
  （Step 1 只对前后两日收盘均有效者计收益）, 权重保留至下一调仓日
  （期间仍占仓位、拿不到现金收益）, 调仓日随池更新自然退出（不计价格
  亏损、换手佣金照计）——即"静默零贡献"而非"冻结价格"。
  本库 NaN 段的真实构成（2026-09-08 抽样审计, 30只/65段）: 未上市填充/
  终止上市尾段/真数据洞(~25%)/停牌, 另有两段跨股票同步的全局数据洞
  （2026-05-11~06-16、2026-07-01~07-06, 落在主窗口内, 见 spec §6.9 ⑬）。
  ffill 仅出现在 lot 可行性计算与 momentum 池风格（当前配置未使用）,
  两者不影响估值语义。
"""
import sys, os
